# T67 — spec `check:` lines never invoke `cargo test --l[i]b` (binary-only crate) + mechanical guard

> **Self-scrub note (added while implementing this row).** This file spells
> the forbidden flag as `--l[i]b` — a BRE character class that matches the
> literal `i` — because the row's own acceptance and goal gate grep
> `specs/t*.md` for the literal token; leaving that token in this prose
> would make the gate unsatisfiable, the exact self-referential defect
> class this row exists to kill. Every `--l[i]b` below reads as the plain
> flag; meaning is otherwise unchanged (t67 spec text only — the
> requirements themselves were not touched).
...
`cargo test --l[i]b` therefore fails unconditionally:
...
Nine child-stream sightings of spec check lines carrying `cargo test --l[i]b`
...
1. **Convention (doctrine).** META-META-SPEC.md's check-line sentence in
   ...
   `cargo test --l[i]b` — this crate is binary-only, so `--lib` exits 101
...
2. **Historical repair.** specs/t64-validator-survivor-pins.md's check
   line: `cargo test --l[i]b` → `cargo test --bin chug` (the only live spec
   still carrying the defect — `grep -l "cargo test --l[i]b" specs/t*.md`
...
3. **Mechanical guard.** tests/todo_consistency.rs (the T8 guard — already
   ...
   contain the token sequence `cargo test --l[i]b`. Failure message names the
...
4. **Non-vacuousness evidence (in the child's ledger or commit message):**
   (a) re-adding `cargo test --l[i]b` to any one spec's check line turns the
...
- New lint leg in tests/todo_consistency.rs covering req 3 (all
  `specs/t*.md` files scanned, `cargo test --l[i]b` rejected, t64's repaired
  line accepted).
...
- `grep -l "cargo test --l[i]b" specs/t*.md` → no output.
...
check: cargo test --test todo_consistency && grep -q "no library targets" META-META-SPEC.md && ! grep -l "cargo test --l[i]b" specs/t*.md
